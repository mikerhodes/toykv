...
mod tests {
    ...
    fn test_seek_to_key_exact_match() {
        ...
        table_builder.add(b"a_apple", &KVWriteValue::Some(b"red fruit")).unwrap();
        table_builder.add(b"b_banana", &KVWriteValue::Some(b"yellow fruit")).unwrap();
        table_builder.add(b"c_cherry", &KVWriteValue::Some(b"small red fruit")).unwrap();
        table_builder.add(b"d_date", &KVWriteValue::Some(b"brown fruit")).unwrap();
        table_builder.add(b"e_elderberry", &KVWriteValue::Some(b"purple fruit")).unwrap();

        let temp_file = tempfile::NamedTempFile::new().expect("Failed to create temp file");
        table_builder.write(temp_file.path()).unwrap();

        // Test seeking to "c_cherry" which exists in the table
        let mut table_iterator = TableIterator::new(temp_file.path().to_path_buf()).unwrap();
        ...
        assert_eq!(first_record.value, KVValue::Some(b"small red fruit".to_vec()));
    ...
    fn test_seek_to_key_non_existent_key() {
        ...
        table_builder.add(b"c_cherry", &KVWriteValue::Some(b"small red fruit")).unwrap();
        table_builder.add(b"e_elderberry", &KVWriteValue::Some(b"purple fruit")).unwrap();
        table_builder.add(b"g_grape", &KVWriteValue::Some(b"green fruit")).unwrap();
    ...
    fn test_seek_to_key_last_key() {
        ...
        assert_eq!(records[0].value, KVValue::Some(b"small red fruit".to_vec()));
    }
    ...
    fn test_seek_to_key_with_deleted_entries() {
        ...
        table_builder.add(b"a_apple", &KVWriteValue::Some(b"red fruit")).unwrap();
        table_builder.add(b"b_banana", &KVWriteValue::Deleted).unwrap();
        table_builder.add(b"c_cherry", &KVWriteValue::Some(b"small red fruit")).unwrap();
        table_builder.add(b"d_date", &KVWriteValue::Deleted).unwrap();
        table_builder.add(b"e_elderberry", &KVWriteValue::Some(b"purple fruit")).unwrap();
        ...
        assert_eq!(second_record.value, KVValue::Some(b"small red fruit".to_vec()));
    }
    ...
    fn test_seek_to_key_large_table() {
        ...
        for i in 0..1000 {
            let key = format!("key{:06}", i);
            let value = format!("value{:06}", i);
            table_builder.add(key.as_bytes(), &KVWriteValue::Some(value.as_bytes())).unwrap();
        }
